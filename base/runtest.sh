...
echo "sudo docker exec -i $PHPFPM_NAME vendor/bin/phpunit"
sudo docker exec -i $PHPFPM_NAME php artisan migrate:refresh --database=mysqltest && sudo docker exec -i $PHPFPM_NAME vendor/bin/phpunit
